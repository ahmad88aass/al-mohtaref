#!/bin/bash
set -e
mkdir -p supabase/functions/sms-man
cat > supabase/functions/sms-man/index.ts << 'EDGEFN_EOF'
...
EDGEFN_EOF
echo "Edge Function written successfully"
echo "|| count: $(grep -o '||' supabase/functions/sms-man/index.ts | wc -l)"
echo "lines: $(wc -l < supabase/functions/sms-man/index.ts)"
